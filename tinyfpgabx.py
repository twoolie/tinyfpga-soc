...
class TinyFPGAProgrammer(GenericProgrammer):
    needs_bitreverse = False

    # The default flash address you probably want is 0x30000; the image at
    # address 0 is for the bootloader.
    def flash(self, port, bitstream_file):
        subprocess.call(["tinyprog", "--pyserial", "-c", port, "--program", bitstream_file])
...
_io = [
    ...
    ("rst", 0, Pins("A7"), IOStandard("LVCMOS33")),

    ("serial", 0,
        Subsignal("tx", Pins("B6")),
        Subsignal("rx", Pins("A6")),
        IOStandard("LVCMOS33")
    ),
...
def main():
    ...
    if build:
        ...
        print("[flashing]...")
        prog = TinyFPGAProgrammer()
        prog.flash(args[1] if len(args) > 1 else '/dev/ttyACM0', "build/gateware/top.bin")
